...
static int xcastmagicy(struct monst *, struct monst *, struct attack *, int, int);
...
static int hmoncore(struct monst *, struct monst *, struct attack *, struct attack *, struct obj **, void *, int, int, int, boolean, int, boolean, int, unsigned long);
...
static boolean blacklight_tentacles_suck_x(struct monst *, struct monst *, int, int, const char *);
		...
		if (notmcan || (u.ustuck == mtmp)) {
			...
			/* if you are attached to the other creature, do the thing! */
				...
				/* Gentle squeeze. Restore 1d3 points of sanity. Just kidding. */
					...
					if(pa->mtyp == PM_OCTOPODE){
						pline("%s %s.",
							youagr ? Monnam(mtmp) : s_suffix(Monnam(mtmp)),
							(youagr ? "is wrapped up in your tentacles" : "tentacles are wrapped tight around you")
							);						
					}
	...
	struct poisoninfo * p)	  /* used to return results */
{
	...
	if (p->poisons)
	{
		...
		{
			i = (1 << n);
			boolean major = (p->majoreff & i) != 0;
			boolean minor = (p->minoreff & i) != 0;
			if (!major && !minor)
				continue;
			/* calculate poison damage */
			switch (i)
