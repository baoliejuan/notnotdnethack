...
static int ansermee_scan(struct obj *);
...
static int use_saw_cleaver(struct obj *);
static int use_saw_spear(struct obj *);
static int use_soldier_rapier(struct obj *);
static int use_bow_blade(struct obj *);
static int use_threaded_cane(struct obj *);
static int use_chikage(struct obj *);
...
static void use_dissection_kit(struct obj *);
...
static boolean uhave_usablestone(void);
...
static void
use_treph_parasites(register struct obj *obj)
{
	winid tmpwin;
	anything any;
	any.a_void = 0;         /* zero out all bits */
